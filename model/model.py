...
# OpenCV 오류 문구 무시
def ignore_cv2_errors():
    def handler(exception_type, exception, traceback):
        if exception_type == cv2.error:
            pass
        else:
...
def extract_frames_from_video(video_path, frame_interval=50):
...
def load_video_frames(video_paths, frame_interval=50):
    all_frames = []
    net, output_layers, classes = load_yolo_model()

    for video_path in video_paths:
        video_frames = extract_frames_from_video(video_path, frame_interval)
        for frame in video_frames:
            people = detect_people(frame, net, output_layers, classes)
            if people:
                all_frames.append((frame, people))
...
async def run_model(user_conditions, video_paths, cctv_idxs, missing_id):
    ...
    for video_path, cctv_idx in zip(video_paths, cctv_idxs):
        ...
        model.eval()
        image_idx = 1
        for frame_idx, (frame, people) in enumerate(frames):
            for person_idx, (person_img, (x, y, w, h)) in enumerate(people):
