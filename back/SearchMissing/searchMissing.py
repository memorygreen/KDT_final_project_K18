...
def search_missing():
    try:
        ...
        missing_location_lat = float(data.get('missing_location_lat'))
        missing_location_lng = float(data.get('missing_location_lng'))
        ...
        # CCTV 정보 가져오기
        nearest_cctvs_info = asyncio.run(nearest_cctvs(missing_location_lat, missing_location_lng))
        if nearest_cctvs_info:
            asyncio.run(update_nearest_cctvs_to_db(missing_id, nearest_cctvs_info))
        
        # 비동기로 딥러닝 서버에 요청을 보내기 위해 threading을 사용하여 백그라운드 작업 생성
        threading.Thread(target=trigger_deep_learning_server, args=(missing_id,)).start()
...
def trigger_deep_learning_server(missing_id):
    try:
        ...
        print(f"Sending request to external server: {external_url}")  # 디버깅 출력을 추가
        async def send_request():
            async with aiohttp.ClientSession() as session:
                payload = json.dumps({"missing_id": missing_id})
                async with session.post(external_url, data=payload, headers={'Content-Type': 'application/json'}) as response:
                    response_text = await response.text()
                    print(f"Response from external server: {response_text}")  # 응답 내용을 출력
                    # 응답이 200이 아닌 경우 에러를 출력
...
async def fetch_cctv_data(session):
    try:
        async with session.get('http://localhost:5000/CCTVLocation') as response:
            ...
            return data
    except aiohttp.ClientError as e:  # 명시적으로 aiohttp.ClientError 사용
