...
const SearchMissing = ({ initialData }) => {
    ...
    // 실종자 이미지 
    const [missingImg, setMissingImg] = useState(null); // 기본 값을 null로 설정
    const [isUploading, setIsUploading] = useState(false); // 업로드 상태 관리 추가
    ...
    const topOptions = [
        { id: 'top long', label: '긴팔' },
        { id: 'top short', label: '반팔' },
    ];

    const topColorOptions = [
        { id: 'top red', label: '빨간색' },
        { id: 'top yellow', label: '노란색' },
        { id: 'top green', label: '초록색' },
        { id: 'top blue', label: '파란색' },
        { id: 'top purple', label: '보라색' },
        { id: 'top white', label: '흰색' },
        { id: 'top gray', label: '회색' },
        { id: 'top black', label: '검정색' },
    ];

    const bottomOptions = [
        { id: 'bottom long', label: '긴바지' },
        { id: 'bottom short', label: '반바지' },
        { id: 'bottom skirt', label: '치마' },
    ];

    const bottomColorOptions = [
        { id: 'bottom yellow', label: '노란색' },
        { id: 'bottom green', label: '초록색' },
        { id: 'bottom blue', label: '파란색' },
        { id: 'bottom purple', label: '보라색' },
        { id: 'bottom pink', label: '분홍색' },
        { id: 'bottom brown', label: '갈색' },
        { id: 'bottom white', label: '흰색' },
        { id: 'bottom gray', label: '회색' },
        { id: 'bottom black', label: '검정색' },
    ];

    const belongingsOptions = [
        { id: 'hat', label: '모자' },
        { id: 'backpack', label: '배낭' },
        { id: 'shoulder bag', label: '숄더백' },
        { id: 'hand bag', label: '핸드백' },
    ...
    // 포스터 생성기능
    const [posterGenerating, setPosterGenerating] = useState(false); // 기본 값을 false로 설정

    const handle_submit = async (event) => {
        event.preventDefault();
        let uploadedImageUrl = ''; // 변수를 함수 범위 내에서 정의

        if (missingImg) {
            try {
                setIsUploading(true); // 업로드 상태 시작
                uploadedImageUrl = await UploadMissingImg(missingImg); // 업로드 완료될때까지 기다리기
                setMissingImgUrl(uploadedImageUrl);
                console.log("업로드된 이미지 URL:", uploadedImageUrl);
                // setMissingImgUrl을 제대로 업데이트한 후 그 값을 로그로 출력
                console.log("업로드된 이미지 URL (확인):", uploadedImageUrl); 
            } catch (error) {
                console.error('실종자 이미지 aws 업로드 실패 Failed to upload image');
                return; // 업로드에 실패하면 이후 코드를 실행하지 않도록 합니다.
            } finally {
            ...
            if (uploadedImageUrl) {
                axios.post('/SearchMissing', {
                    ...
                    missing_img_url: uploadedImageUrl,
